...
const Testimonial = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setSecondaryIndex(
        (prevSecondaryIndex) => (prevSecondaryIndex + 1) % testimonials.length
      );
    }, 5000);
  ...
  const getVisibleTestimonial = (index) => testimonials[index];
    ...
    <div className="relative w-[90%] m-auto overflow-hidden py-10">
      ...
      <div className="flex flex-col md:flex-row justify-center items-center space-y-4 md:space-y-0 md:space-x-4">
        ...
        <div className="flex w-full md:h-40 lg:h-80 md:w-1/2 p-4 md:p-4 rounded-md bg-red-800 text-white">
          <div className="flex-shrink-0 w-1/3">
            <Image
              src={getVisibleTestimonial(currentIndex).image} // Dynamically passed image source
              alt={getVisibleTestimonial(currentIndex).name}
              width={96}
              height={96}
          ...
          <div className="flex flex-col w-2/3 ml-4">
            <h2 className="font-bold text-left text-lg text-white">
              <div className="flex gap-20">
                {getVisibleTestimonial(currentIndex).title}
                <RiDoubleQuotesR className="text-xl font-bold" />
              </div>
            </h2>
            <p className="mt-1 text-lg mb-5 text-left">
              - {getVisibleTestimonial(currentIndex).name}
            </p>
            <p className="text-sm text-left mb-4">
              {getVisibleTestimonial(currentIndex).content}
            </p>
          </div>
        </div>
        ...
        <div className="hidden shadow-lg md:h-40 lg:h-80 border-b-4 border-red-500 md:flex w-full md:w-1/2 p-4 rounded-md bg-white text-gray-900">
          <div className="flex-shrink-0  w-1/3">
            <Image
              src={getVisibleTestimonial(secondaryIndex).image} // Dynamically passed image source
              alt={getVisibleTestimonial(secondaryIndex).name}
              width={100}
              height={100}
          ...
          <div className="flex flex-col w-2/3 ml-4">
            <h2 className="font-bold text-left text-lg">
              <div className="flex gap-20">
                {getVisibleTestimonial(secondaryIndex).title}
                <RiDoubleQuotesR className="text-xl text-red-800 font-bold" />
              </div>
            </h2>
            <p className="mt-1 text-md mb-3 text-left">
              - {getVisibleTestimonial(secondaryIndex).name}
            </p>
            <p className="text-sm text-left mb-4">
              {getVisibleTestimonial(secondaryIndex).content}
            </p>
          </div>
        </div>
      </div>
      ...
      <button
        ...
        onClick={() => {
          ...
          setSecondaryIndex(
            (prevSecondaryIndex) =>
              (prevSecondaryIndex - 1 + testimonials.length) %
              testimonials.length
          );
        }}
      >
